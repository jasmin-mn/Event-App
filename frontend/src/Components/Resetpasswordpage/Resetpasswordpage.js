...
function Resetpasswordpage() {
  ...
  const sendResetToken = async () =>{
    ...
    try {
      const response = await axios.get(`http://localhost:7000/user/newPassword/${token}` )
  ...
  const sendResetPw = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:7000/user/newPassword', user,config)
      console.log(response.data.msg);
      addModalBox("You have successfully reset your password")
